...
// built an unbounded ack loop in production: developer says "Ack, standing
// down" -> Lead's own inbox now has an unread message so ITS prompt says
// reply -> "Confirmed, thanks" -> which is a new unread message for the
// developer -> "Ack, thanks" -> "Sounds good" -> "Understood, thanks" ...
// 20+ paid cycles and millions of cache-read tokens spent acknowledging
// acknowledgments, never converging. A message being unread just means
// nobody's looked at it yet — check_inbox marking it read already *is*
// the acknowledgment; it doesn't also require a reply.
const NO_ACK_LOOPS =
